...
public class TypeChecker implements Absyn.TypeVisitor
{
    ...
    public Type visit(Absyn.Program ast)
    {
		...
		*/
			...
			if (classEnv.get(c.name) != null) {
            	error(c, "Duplicate class definition for " + c.name);
        	} else {
            	CLASS newClass = new CLASS(c.name);
            	classEnv.put(c.name, newClass); //add new class to class table
        	}
		...
		*/
			CLASS childClass = classEnv.get(c.name); //inheritance if a parent is specified
        
			if (c.parent != null) {
				CLASS parentClass = classEnv.get(c.parent);
				if (parentClass == null) {
					error(c, "Parent class " + c.parent + " not found for class " + c.name);
				} else {
					childClass.parent = parentClass; //set parent-child link
				}
			}

			//visit fields and methods to add them to the class environment
			varEnv.beginScope(); 
		...
		*/
		...
		CLASS currentClass = classEnv.get(c.name);

        if (isLoop(currentClass)) {  //check for inheritance cycles
            error(c, "Inheritance cycle detected for class " + c.name);
            continue;
        }

        mergeParents(currentClass.parent, currentClass.instance); //merge inherited fields and methods
    ...
    public Type visit(Absyn.ClassDecl ast) 
    {
		...
		if (classEnv.get(ast.name) != null) {
			error(ast, "Duplicate class definition for " + ast.name);
			return null; //null to indicate an error
		}

		CLASS newClass = new CLASS(ast.name); //create new class
		classEnv.put(ast.name, newClass); //put class in class env
...
//
    ...
    { 
		...
		*/
		...
		Type fieldType = ast.type;
		FIELD field = new FIELD(fieldType, ast.name);
	
		if (currentClass.fields.containsKey(ast.name)) {
			error(ast, "Duplicate field " + ast.name + " in class " + currentClass.name);
		} else {
			currentClass.fields.put(fieldType, ast.name);  //add the field to the class
		}
